...
public class Prob8 {
	 ...
	 * @param args
	 */
	// cherche le maximum du prod de 5 chiffre consecutif (marche, rep 40824)
	// chaine de caractere a entrer (les espaces ont ete rajouter)
	/**
	...
	// rentre les chiffredans un tableau
	...
	// effectue le produit de tous les 5 chiffres conscutifs et trouve le max
	public int traiter(int[] tab) {
		int max = 0;
		for (int i = 0; i < tab.length - 4; i++) {
			int prod = tab[i] * tab[i + 1] * tab[i + 2] * tab[i + 3]
					* tab[i + 4];
			if (prod > max)
				max = prod;
		}
